...
use sled::{transaction::TransactionResult, Db, Error as DbError};
...
impl<'a> BlockChain<'a> {
    pub fn new(db: &'a Db) -> Result<Self, DbError> {
        let result: TransactionResult<String> = db.transaction(|db| {
            let tip = match db.get(LAST_HASH)? {
                ...
                None => {
                    ...
                    let serialized = rmp_serde::to_vec(&genesis).expect("Should serialize a block");
...
impl<'a> BlockChain<'a> {
    pub fn add_block(&mut self, data: &str) -> Result<(), DbError> {
        ...
        let result: TransactionResult<()> = self.db.transaction(|db| {
